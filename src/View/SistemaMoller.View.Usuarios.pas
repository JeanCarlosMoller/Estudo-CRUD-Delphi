unit SistemaMoller.View.Usuarios;
...
uses
  ...
  Vcl.ExtCtrls, Vcl.WinXPanels, Vcl.WinXCtrls, Vcl.Menus, SistemaMoller.View.CadastroPadrao;
...
begin
  ...
  begin
    edtNome.SetFocus;
    Application.MessageBox('Por favor, preencha o campo nome.', 'Atenção', MB_OK + MB_ICONWARNING);
    lblNome.Caption := '*';
  ...
  begin
    edtLogin.SetFocus;
    Application.MessageBox('Por favor, preencha o campo login.', 'Atenção', MB_OK + MB_ICONWARNING);
    lblLogin.Caption := '*';
    lblLogin.font.Color := clRed;
    abort;
  end;
  ...
  begin
    edtLogin.SetFocus;
    Application.MessageBox(PWideChar(Format('Login %s já cadastrado no sistema.', [edtLogin.Text])), 'Atenção', MB_OK + MB_ICONWARNING);
    abort;
  end;
